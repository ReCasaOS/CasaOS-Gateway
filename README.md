...
- **The management API**, on `127.0.0.1` at a port the kernel assigns. `GET` and `POST /v1/gateway/routes` list and register routes; `GET` and `PUT /v1/gateway/port` read and change the public port. All four need either a person's CasaOS JWT or, from loopback, `Authorization: Internal <secret>`, where the secret is the one the gateway writes to `/var/run/casaos/internal.secret` (root only, mode 0600) at every start. Coming from loopback is not enough on its own: any local process can do that. The address is written to `/var/run/casaos/management.url`, which is how the other services find it, and the port endpoint is also proxied on the public port under `/v1/gateway/port`.
...
- **Its own module path.** The Go module is `github.com/ReCasaOS/CasaOS-Gateway` and it builds against `github.com/ReCasaOS/CasaOS-Common v0.4.22`, so log lines, stack traces and `go version -m` on the shipped binary name this fork instead of the upstream that stopped shipping.
